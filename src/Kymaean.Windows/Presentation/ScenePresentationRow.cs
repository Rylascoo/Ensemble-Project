    ...
    IReadOnlyList<CharacterPresentationRow> Roster)
{
    ...
    public string CodeLabel =>
        Code is null ? "Scene code unavailable" : $"Scene code {Code}";
    ...
    public string InspectName =>
        $"Inspect initial roster for {CodeLabel}";
    public string DetailHeadingName =>
        $"Initial roster. {CodeLabel}";
    ...
    public static IReadOnlyList<ScenePresentationRow> Build(
        ...
        Build(replay, cast, PresentationIdentityCode.BuildSceneCodes);

    internal static IReadOnlyList<ScenePresentationRow> Build(
        ProductionReplayProjection replay,
        IReadOnlyList<CharacterPresentationRow> cast,
        Func<IReadOnlyList<string>, IReadOnlyDictionary<string, string?>> buildCodes)
    {
        ArgumentNullException.ThrowIfNull(replay);
        ArgumentNullException.ThrowIfNull(cast);
        ArgumentNullException.ThrowIfNull(buildCodes);

        var codes = buildCodes(
            replay.ProductionScenes.Scenes
                .Select(scene => scene.Id.Value)
                .ToArray());
        var characters = cast.ToDictionary(character => character.Id);

        return replay.ProductionScenes.Scenes
            .Select(
                scene =>
                    new ScenePresentationRow(
                        scene.Id,
                        codes[scene.Id.Value],
