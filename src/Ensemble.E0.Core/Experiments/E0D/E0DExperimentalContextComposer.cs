...
internal static class E0DExperimentalContextComposer
{
    private const string OpportunityText = "You have the current opportunity to act.";

    internal static ContextCompositionEvaluation Compose(
        ...
        string compositionContract)
    {
        ...
        if (recentPerformances.IsDefault)
        {
            throw new ContextCompositionException(
                "E0-D experimental Context recent Performance history is uninitialized.");
        }
        if (string.IsNullOrWhiteSpace(compositionContract))
        {
            throw new ContextCompositionException(
                "E0-D experimental Context composition contract is required.");
        }

        var hasAcceptedHistory = recentPerformances.Length != 0;
        return ComposeCore(
            projection,
