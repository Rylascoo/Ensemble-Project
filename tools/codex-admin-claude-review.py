...
def process_runner(
    ...
    try:
        ...
        raise DispatchError(
            f"Claude subprocess exceeded {PROCESS_TIMEOUT_SECONDS}s hard bound"
        ) from exc
...
def review_command(claude_exe: Path) -> list[str]:
    schema = json.dumps(REVIEW_SCHEMA, separators=(",", ":"), sort_keys=True)
    return [
        str(claude_exe), "-p",
        "--safe-mode",
        "--restricted",
...
def extract_structured(envelope: dict[str, object]) -> dict[str, object] | None:
    direct = envelope.get("structured_output")
...
def dispatch(request: DispatchRequest, runner: Runner = process_runner) -> dict[str, object]:
    ...
    try:
        ...
        raise DispatchError("Claude review output was not JSON") from exc
    if not isinstance(envelope, dict):
        raise DispatchError("Claude review output JSON was not an object")
    structured = extract_structured(envelope)
    if not isinstance(structured, dict):
        raise DispatchError("Claude review did not return structured output")

    findings = structured.get("findings")
    verdict = structured.get("verdict")
    valid_structured = (
        verdict in {"PASS", "PASS_WITH_FINDINGS", "FAIL"}
        and isinstance(structured.get("summary"), str)
    ...
    passed = bool(
        ...
        and unchanged
        and valid_structured
        and provider == "firstParty"
        and num_turns == 1
        and permission_denials in ([], None)
        and not envelope.get("is_error", False)
    )

    telemetry: dict[str, object] = {
        "schema": "ensemble.c9a-claude-review.v1",
        "pass": passed,
        "repo": str(repo),
    ...
    if not passed:
        raise DispatchError(
            "C9A review failed closed; inspect scratch telemetry/result without retrying automatically"
        )
    return telemetry
