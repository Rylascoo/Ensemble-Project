...
Phase: **E0-A Experimental Harness — promoted main retains historical OpenAI credentialless native authority; the Gemini normative-reference amendment has a credentialless native Windows ARM64 PASS on its live branch pending Director promotion/disposition; post-audit hardening has a repaired test-oracle candidate requiring a full native rerun; real provider execution remains gated**.
...
Durable annotated validation tags now include:
...
### Gemini normative-reference amendment checkpoint — live branch, not yet promoted
...
This is native credentialless/fake-only authority for the exact Gemini branch checkpoint. It is **not** a merge/promotion decision and does not authorize Gemini `countTokens`, inference, credential use, provider-network execution, or spend.

Coverage caveat: the pricing-policy tuple test `PricingPolicy_FreezesSourcePromotionAndConservativeRates` is a **constant-freeze tripwire**, not behavioral pricing validation. Its pass contributes to the historical OpenAI suite count but only proves that the compiled frozen constants match the independently maintained expected tuple in the test. It does not establish live provider pricing freshness, provider cache behavior, or runtime spend correctness. Those claims require separate runtime/provider evidence and remain subject to known pricing-snapshot limitations.

The comparison from `1cfdb3aa22abce62a5bd48e80706407670d1c6a9` to former branch head `9228c77a5b789c4c6a1e0417f889851c6b92730e` contains documentation only: `CURRENT_STATE.md`, `docs/PROJECT_AUTHORITY.md`, four live-host evidence/host records, and this handoff. The four parallel-main commits after merge base are also documentation-only. The reconciliation therefore changes no executable, test, fixture, or Core surface and does not replace `1cfdb3aa...` as the promoted-main machine-tested live-host executable/test authority.
...
The Harness owns the approved reference-run envelope outside Core while reusing the closed H1 Cycle/Turn authority unchanged. Core remains unchanged by the promoted historical live-host completion.

Frozen historical provider/model control on promoted `main`: **OpenAI / `gpt-5.6-sol` only** for E0-A under Blueprint law #35. The separately approved Gemini normative-reference amendment is carried on its live branch and is not silently treated as merged project source merely because its exact branch checkpoint has native validation evidence.

Historical approved OpenAI live variants are exactly:
...
Frozen operational limits remain: 12 accepted-Turn cap; one attempt per probabilistic invocation; zero automatic retries; 300-second attempt timeout; 4096 max output tokens per role; estimated model-token spend ceiling USD 5.00/run; `store=false`; no tools/provider conversation/hidden reasoning. Provider-specific request/pricing/cache controls remain governed by the exact approved route and its evidence rather than being generalized across providers.

The live host validates exact clean checkout, evidence-root absence, fixture/Missing-Raft contract, and applicable pricing date validity before credential access. It exposes explicit `e0a-run` and independent post-run `e0a-evaluate` paths and preserves write-once/runtime-root evidence sealing.

The historical OpenAI host's `RequireNonStaleSnapshot` remains a date-validity guard, not live pricing verification. The Gemini amendment has its own short-lived provider-contract/pricing/data-use snapshot and fail-closed freshness boundary. Neither is live pricing verification.

Closing those residuals still requires the previously identified trusted re-verification discipline or an equivalent stable machine-readable provider source. Native credentialless PASS evidence does not erase or narrow that known limit.
...
- `e0a-phase-b-post-audit-hardening`
  - Documentation/evidence branch head after Attempt 01 diagnosis: `ab65d3c70887c80c1e9c3ce068682e4eb79539a8`.
  - Exact repaired source/test checkpoint requiring the next full native rerun: `5c70f619d6e951d89bb527a5945b014998573dab`.
  - Merge base with `main`: `f9cb1a79ad7923b640ff1a97c46d8fd6ab9dac25`; branch currently carries **69 unique commits** over that merge base.
  - **OPEN — NATIVE ATTEMPT 01 FAILED 87/88 HARNESS TESTS; STALE TEST ORACLE IDENTIFIED AND REPAIRED; FULL RERUN REQUIRED; DIRECTOR DISPOSITION AFTER EVIDENCE.**
  - The earlier MSTEST0032 compiler blocker was closed by `dc7ac428...`; native Attempt 01 proved the Harness and Harness tests compiled and entered native ARM64 execution.
  - Attempt 01 then failed only `BufferedWrongJsonType_FailsClosedAsTechnicalReceipt`: expected `provider-incomplete`, actual `malformed-provider-response`.
  - Repository history shows E-06 commit `ce23987c12431295ac78726d36c7cf570ffd02ad` intentionally refined malformed-response diagnostics/provenance after the E-03 regression was written. The product implementation is consistent with the later contract; the test oracle was stale.
  - Repair `5c70f619...` changes one expected diagnostic string only. No `src/**`, Core, fixture, project configuration, or runtime behavior changed. Full native rerun is still required because Attempt 01 stopped before the explicit fresh-build/smoke/credentialless stages.
  - Failure/diagnosis evidence: `docs/evidence/E0A_POST_AUDIT_HARDENING_NATIVE_ARM64_VALIDATION_ATTEMPT_01.md` on the branch.
...
- `e0a-gemini-normative-reference-amendment`
  ...
  - Merge base with `main`: `f9cb1a79ad7923b640ff1a97c46d8fd6ab9dac25`; branch currently carries **50 unique commits** over that merge base.
  - **OPEN — CREDENTIALLESS NATIVE WINDOWS ARM64 VALIDATION PASS; DIRECTOR PROMOTION/DISPOSITION AND BRANCH-TO-MAIN RECONCILIATION PENDING.**
  - Validation: Core **622/622**, Harness **81/81**, fresh Harness native ARM64 build PASS, Missing Raft PASS, generic smoke PASS, credentialless Gemini provider-edge PASS with both credentials absent and no evidence root.
  - Durable locator: `validation/e0a-phase-b-gemini-native-arm64` -> `9bb65a846...`.
  - Evidence: `docs/evidence/E0A_PHASE_B_GEMINI_NATIVE_ARM64_VALIDATION.md` on the branch.
  - This PASS does not authorize real Gemini network execution, credentials, or spend.
...
The immediate executable-authority gate is a **full Director-machine native Windows ARM64 rerun of hardening checkpoint `5c70f619d6e951d89bb527a5945b014998573dab` from the beginning**. A focused rerun of the previously failing test is not promotion evidence.

The Gemini branch requires no repeat of the credentialless native run solely to prove `9bb65a846...`; that exact checkpoint is already tagged and machine-tested. Its next engineering step is branch-to-main reconciliation and Director promotion/disposition, while preserving its provider-network gate.

After hardening either passes or produces new failure evidence, perform Director disposition of both live implementation branches against current `main`. Do not silently merge, discard, or compose the two branches: each changes the E0-A provider/behavior surface from the same historical baseline and requires explicit reconciliation.
