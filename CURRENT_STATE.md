...
## Checkpoint
Runtime **E0 experimental program — Q-E0D-01 ablation controls**. E0-A/E0-B/E0-C are **DONE**. E0-D is **ACTIVE — P01 SLOT 1 AUTHORIZED; PROJECT/KEY ASSOCIATION ROTATED; PROTECTED LOCAL REBIND PASS; WINDOW/CAPACITY GATES PENDING; NAMESPACE UNCONSUMED; PROVIDER TRAFFIC ZERO**.
...
## Continuity
PR #147 closed activation integration at exact-main `e8b702a5b37d208d7caeeb6ffeb5a6beacf24f85` with Validation #855 PASS. Director amendment `docs/evidence/E0D_P01_SLOT1_PROVIDER_ASSOCIATION_ROTATION_DIRECTOR_AMENDMENT_2026_09_15.md` supersedes only the prior authenticated project/key association for future P01 execution: project `Ensemble Testing`, project ID `gen-lang-client-0793779417`, credential label `Gemini API Key`, Free tier. Local readiness evidence `docs/evidence/E0D_P01_SLOT1_PROTECTED_CREDENTIAL_REBIND_READINESS_2026_09_15.md` records the fresh key as protected through the established CurrentUser mechanism with provider traffic zero; the secret remains outside chat/Git and the old protected credential remains ineligible for P01. RunId/root/model/profile/fixture/executable/window/no-retry law remain unchanged. Later slots remain separately unauthorized; Q-E0E-RUN remains blocked.

## Next
Stop before claim/provider traffic until UTC is inside `2026-09-16T14:30:00Z..23:30:00Z`. At the launch boundary, re-run the non-consuming machine census, freshly confirm the amended project/key association, exact model, Free tier and sufficient RPM/TPM/RPD capacity, recheck every frozen interlock and verify the new protected credential is selected; then launch `E0D-Q01-P01-FULL-20260914-01` exactly once only if all gates pass. Do not activate P01 Slot 2 or any later slot without separate Director authority.
