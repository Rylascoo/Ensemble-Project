...
Blueprint 0.1 is frozen for E0. The runtime phase is **E0-A Phase B**. E-R1 repository/tooling/workflow restructuring is **closed and archived**. The Gemini Free-tier RPD/model-selection work and first-real-attempt `countTokens` correction are native-validated and promoted at tagged Windows ARM64 checkout `689655eed677b789ab3ee395f1c65b4f2cb72cc8`. Real Gemini 3.5 Flash-Lite attempts 01 and 02 are consumed technical failures at zero accepted turns; attempt 02 reached first Performer `countTokens` and received HTTP 401 before generation. A later Director diagnostic proved a fresh `AQ.` key can reach Gemini 3.6 generation via `?key=`. The subsequent `gemini-3.5-flash-lite:countTokens` transport differential is consumed: its query leg was locally inconclusive, while the same fresh unshared key succeeded via `x-goog-api-key` with HTTP 200 and `totalTokens=8`, so no auth-transport patch is justified. Third Missing Raft attempt 03 used that same known-good unshared key at promoted `689655...`; all local preflights passed, the single authorized host invocation started and terminated `TechnicalFailure` at 0 accepted turns / $0 shadow spend, and its 8-file evidence archive is now the sole audit gate. Provider authorization is none; no retry, fourth run, fallback, or other provider traffic is authorized. H1 Phase A is **CLOSED**.
