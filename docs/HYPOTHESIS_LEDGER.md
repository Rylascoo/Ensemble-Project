...
Current evidence: Run 02 plus bounded diagnostics localized and corrected the 3.5 structured-output encoding. Run 03 completed 21 real generations with stable model identity and six accepted turns before a local Interpreter semantic rejection. Run 04 exposed the canonical control-ID interface gap. Run 05 proved that correction reached Performer acceptance and Integrity, then ended on the former catch-all Interpreter technical-failure path after successful `countTokens`. Run 06 exercised the hardened diagnostic: Performer `countTokens` succeeded at 724 input tokens and the first generation returned HTTP 503 / `UNAVAILABLE`, with no response ID, returned model, usage receipt, or structured output. Evidence: the Run 03/04/05/06 terminal analyses.

Disposition: repeated prior 3.5 Flash-Lite transport and structured-output generation remain established. Run 06 narrows its own terminal event to a provider HTTP availability-class failure and proves the diagnostic hardening worked; it does not support parser, canonical-ID, credential, quota-429, or local semantic failure claims. Runs 03/04/05/06 remain noncontributing and immutable. 3.1 Flash-Lite and 2.5 Flash-Lite live generation compatibility remain separately unverified.

Verification trigger: Run 07 now has a fresh reserved 3.1 RunId/evidence root and current public model/lifecycle/Free-tier/pricing/data-use/capability plus exact native/profile/fixture/credentialless preactivation evidence. Before traffic, complete only the authenticated AI Studio project/key/RPM/TPM/RPD gate and durably integrate the final activation boundary. Do not invent a compatibility-only provider probe unless evidence makes it necessary.

Falsifier/narrowing evidence: a separately authorized fresh 3.1 full-candidate run reveals a provider/API incompatibility or cannot satisfy the frozen activation/contribution gates.
...
For 3.1 Flash-Lite, current Google documentation rechecked 2026-09-11 confirms the exact model has Gemini API Free-tier access, but active RPM/TPM/RPD are project/model/tier specific and must be read in AI Studio. Run 07 is reserved and every non-authenticated preactivation gate passes, but the intended project tier, protected authorization-key/project association, and current 3.1 quota row remain unresolved. The commissioned browser is deliberately Guest-only and cannot inspect that authenticated state. The recorded 15 RPM / 250,000 input TPM / 500 RPD values remain historical input to the executable/profile, not current Run 07 account evidence. For 2.5 Flash-Lite, current account/quota state remains separately unverified.

Verification trigger: before each provider execution, apply the exact route's current decision law. For reserved Run 07, use the intended authenticated AI Studio project to verify Free tier, protected key/project association, and exact 3.1 RPM/input-TPM/RPD. If those values differ from the encoded 15 / 250,000 / 500 snapshot, stop for executable/profile audit rather than probing the provider. For later 2.5 work, reverify its own account/project/model/quota facts immediately before execution.
...
Current evidence: Run 03 completed 21 generation responses across Performer/Integrity/Interpreter with stable returned model `gemini-3.5-flash-lite`. Generation receipts report 36,222 input tokens, 6,121 output tokens including 4,811 reasoning tokens, and zero cached input; the Harness reconciled spend with `hasUnknownProviderUsage=false`. Run 05 correctly retained the full Interpreter reservation and marked `hasUnknownProviderUsage=true` when generation failed before a usage receipt. Run 06 repeated that fail-closed accounting behavior on the first Performer generation: HTTP 503 / `UNAVAILABLE`, no usage receipt, full USD 0.16405720 shadow reservation retained, and `hasUnknownProviderUsage=true`. This preserves conservative accounting while leaving exact failed-call consumption unknown.
...
- Run 03 established repeated 3.5 transport/accounting success and localized an Interpreter semantic mismatch; Run 04 localized the canonical-ID interface gap; Run 05 proved that correction reached Performer/Integrity and then exposed the former catch-all Interpreter technical-failure diagnostic; Run 06 proved the hardened diagnostic path and terminated on first Performer generation HTTP 503 / `UNAVAILABLE`. No source correction or automatic 3.5 retry is justified. Run 07 is now reserved for 3.1 with public/native preactivation passing; provider traffic remains unauthorized until the authenticated AI Studio project/key/quota gate is durably satisfied.
