...
Current evidence: Run 02 plus bounded diagnostics localized and corrected the 3.5 structured-output encoding. Run 03 completed 21 real generations with stable model identity and six accepted turns before a local Interpreter semantic rejection. Run 04 then exposed the canonical control-ID interface gap. Run 05 proved that correction reached Performer acceptance and Integrity, but its first Interpreter generation ended in a catch-all `gemini-malformed-or-transport` technical failure after successful Interpreter `countTokens`; no response ID, usage receipt, or evidence-eligible stream event survived. Evidence: the Run 03/04/05 terminal analyses.

Disposition: repeated 3.5 Flash-Lite transport and structured-output generation remain established, but Run 05 narrows a new unresolved first-event technical-failure class at the Interpreter boundary. Its evidence cannot distinguish HTTP transport, response I/O, malformed JSON/UTF-8, or defensive overflow. Runs 03/04/05 remain noncontributing and immutable. 3.1 Flash-Lite and 2.5 Flash-Lite live generation compatibility remain separately unverified.

Verification trigger: first native-validate/tag/integrate the zero-provider technical-failure diagnostic classification amendment. Only afterward may one fresh 3.5 reference RunId be preregistered; that run tests contribution/reliability and may also narrow the Run 05 technical class if it recurs. Later model families retain their own live-compatibility requirements.
...
Current evidence: Run 03 completed 21 generation responses across Performer/Integrity/Interpreter with stable returned model `gemini-3.5-flash-lite`. Generation receipts report 36,222 input tokens, 6,121 output tokens including 4,811 reasoning tokens, and zero cached input; the Harness reconciled spend with `hasUnknownProviderUsage=false`. Run 05 added successful Performer/Integrity receipts, then correctly retained the full Interpreter reservation and marked `hasUnknownProviderUsage=true` when the generation failed before any usage receipt. This preserves conservative accounting while leaving the exact final-call consumption unknown.
...
- Run 03 established repeated 3.5 transport/accounting success and localized an Interpreter semantic mismatch; Run 04 localized the canonical-ID interface gap; Run 05 proved that correction reached Performer/Integrity and then terminated on an unresolved catch-all Interpreter technical-failure diagnostic. The current zero-provider gate is diagnostic classification hardening, not a provider retry.
