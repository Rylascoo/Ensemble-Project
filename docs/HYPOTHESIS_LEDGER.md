...
Verification trigger: Option A is now durable and Run 08 is the one authorized 3.5 Standard/Free replacement. After durable activation integration and green post-merge Validation, execute it exactly once; do not invent a compatibility-only probe, retry, fallback, or second Standard/Free replacement.

Falsifier/narrowing evidence: Run 07 narrowed 3.1 compatibility positively but failed the 12-turn contribution gate on a provider 503. Run 08 is the bounded replacement test of the transient-availability diagnosis; its terminal result will narrow the route evidence without erasing prior failures.
...
Verification trigger: Run 08 activation reuses the Director-approved 3.5 Free-tier snapshot 15 RPM / 250,000 input TPM / 500 RPD because no revalidation trigger has occurred; immediately before traffic recheck for any material account/project/key/model/tier/quota change or provider `429`/`RESOURCE_EXHAUSTED` signal.
...
- Run 03 established repeated 3.5 transport/accounting success; Runs 04/05 localized later local contract/diagnostic defects; Run 06 terminated on first 3.5 Performer generation HTTP 503 / `UNAVAILABLE`; Run 07 established repeated 3.1 transport/structured-output/accounting success and two accepted turns before another HTTP 503 / `UNAVAILABLE`. Option A is now the durable Director route decision and authorizes exactly one fresh 3.5 Standard/Free replacement, Run 08, under the unchanged zero-retry envelope and hard one-replacement stop. The 2.5 control cannot substitute and remains outside this approval.
