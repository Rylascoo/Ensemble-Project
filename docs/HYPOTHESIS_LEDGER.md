...
Verification trigger: Run 07 now has a fresh exact 3.1 RunId/evidence root; current public/provider, native/profile/fixture, authenticated project/key/quota, secure-credential and standing-authority activation gates pass. After durable activation integration and green post-merge Validation, execute the full candidate exactly once; do not invent a compatibility-only provider probe.
...
For 3.1 Flash-Lite, current Google documentation rechecked 2026-09-11 confirms the exact model and Free-tier route. Director-supplied authenticated AI Studio evidence on 2026-09-11 confirms `Gemini Project - Kymaean` is Free tier, `Gemini API Key - testing` is associated with that project, and the exact `Gemini 3.1 Flash Lite` row is 15 RPM / 250,000 input TPM / 500 RPD with zero displayed current usage. Those values exactly match the executable/profile. For 2.5 Flash-Lite, current account/quota state remains separately unverified.

Verification trigger: before each provider execution, apply the exact route's current decision law. Run 07's 3.1 account/project/key/quota gate is satisfied on 2026-09-11 and remains subject to material-change/freshness invalidation before execution. For later 2.5 work, independently reverify its account/project/model/quota facts immediately before execution.
