...
Verification trigger: follow the frozen comparison order. The next lawful work is 3.1 Flash-Lite preactivation: independently reverify current model/lifecycle, Free-tier route, account/project/key/quota, pricing/data-use, and executable/profile compatibility, then establish a fresh exact RunId/evidence root and applicable authority before any traffic. Do not invent a compatibility-only provider probe unless evidence makes it necessary.
