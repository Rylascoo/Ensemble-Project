...
Current evidence: Run 02 plus four bounded diagnostics localized and corrected the 3.5 structured-output encoding. Run 03 then completed 21 real generations with stable model identity and six accepted turns before a local Interpreter semantic rejection. Run 04, using the native-validated Interpreter correction, again passed `countTokens` and generation but its first schema-valid Performer output used display-style IDs because the exact non-subject roster ID vocabulary was not exposed. Evidence: `docs/evidence/E0A_Q_E0A_03_G35L_RUN03_TERMINAL_EVIDENCE_ANALYSIS_2026_09_10.md` and `docs/evidence/E0A_Q_E0A_03_G35L_RUN04_TERMINAL_EVIDENCE_ANALYSIS_2026_09_10.md`.

Disposition: 3.5 Flash-Lite transport, stable returned-model identity, repeated structured-output generation, and legacy GenerateContent encoding remain established. Runs 03 and 04 are noncontributing because local semantic interfaces failed closed; neither result reopens basic 3.5 transport compatibility. 3.1 Flash-Lite and 2.5 Flash-Lite live generation compatibility remain separately unverified.

Verification trigger: only after the role control-identity correction is cleanly native-validated, tagged, integrated, and freshly activated may a new 3.5 reference candidate test contribution/reliability; this does not retest basic transport compatibility. Later model families retain their own live-compatibility requirements.
...
- Run 03 established repeated 3.5 transport/accounting success but terminated on a local Interpreter cross-field semantic mismatch; its correction was native-validated at `e052ef4cdaf8ac7291971f8c42d121e1ee6eecf0`. Run 04 consumed that executable and then failed closed on a distinct Performer canonical-ID interface gap; the role control-identity correction is the current zero-provider-traffic engineering gate.
