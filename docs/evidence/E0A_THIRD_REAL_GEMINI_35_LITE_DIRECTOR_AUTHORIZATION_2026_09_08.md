...
> I confirm the fresh Auth key that passed the `x-goog-api-key` `gemini-3.5-flash-lite:countTokens` diagnostic remains active and unshared, and I authorize exactly one third real synthetic Missing Raft `GEMINI-3.5-FLASH-LITE-MINIMAL` reference run at promoted checkout `689655eed677b789ab3ee395f1c65b4f2cb72cc8`, using that same known-good key, with one attempt per role, zero retries, and no fallback or other provider traffic.
