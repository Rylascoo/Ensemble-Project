...
|---|---|---|---|---|
| Presentation seam | Product / Architecture / Implementation | ACTIVE — BOUNDED DECISION | Determine whether a small platform-neutral Presentation separation materially improves current Windows testability/quality. Falsifier: if the existing `Kymaean.Windows/Presentation` boundary already provides the useful isolation, record no split and move on. | No Product-semantic change, no cross-platform UI implementation, no broad architecture program. One bounded decision/implementation package; then architecture analysis stops absent executable falsification. |
| Native ARM64 CI / performance baseline | Implementation | ACTIVE — MEASUREMENT | Establish a repeatable native ARM64 target-machine validation path and baseline current build/test/startup/runtime behavior before optimization. | Measurement and CI discipline only. No NPU/local-AI correctness dependency, no unmeasured optimization claim, no WACK/Store promotion. |
