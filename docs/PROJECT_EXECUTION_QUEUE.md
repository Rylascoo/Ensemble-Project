...
| ID | Lane | Status | Work | Prerequisite / hard boundary | Exit / successor |
|---|---|---|---|---|---|
| Q-DESIGN-01 | Design Sol | ACTIVE | Continue DPSC-RSP-01 / DPSC-C01 from exact design state after Revision 1 pre-render rejection: restart D0-D5 using a lateral body-height continuous changed condition instead of a ground/support-plane carrier, then recursively test P1-P13 before any scaffold. | `Rylascoo/Ensemble-Website/CURRENT_STATE.md` is design authority. Revision 1 is rejected pre-render; renderer expenditure remains 0 of 2 maximum. C02 is unavailable before a judgeable E1 failure. No scaffold, renderer-visible bytes, sterile packet, or execution gate is authorized until corrected D0-D5 earns its next gate. | Corrected DPSC-C01 is either rejected under frozen P1-P13 or earns a new candidate-specific neutral-scaffold falsification gate; central queue is reconciled again when that status materially changes. |
