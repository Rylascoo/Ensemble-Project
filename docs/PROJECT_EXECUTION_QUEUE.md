...
| Q-E0A-02 | Engineering / Evidence | DONE | Ingest and recursively audit the preserved Attempt-04 evidence ZIP. | Attempt-04 authorization consumed; provider authorization NONE; no retry/fallback/probe. | Closed by `docs/evidence/E0A_GEMINI35_ATTEMPT04_ARCHIVE_AUDIT_2026_09_09.md`: ZIP/seal valid; first Performer `countTokens` HTTP 400 / `INVALID_ARGUMENT`; rejected field `generate_content_request.generation_config.response_format.text.mime_type`; no generation/accepted history/spend. Successor Q-E0A-04 is ACTIVE. |
| Q-E0A-04 | Engineering | ACTIVE | Correct Gemini Developer API `countTokens` to an input-semantic projection and validate it without provider traffic. | Frozen contract `docs/blueprint/E0A_GEMINI_COUNTTOKENS_INPUT_PROJECTION_CORRECTION_AMENDMENT.md`; provider authorization NONE; generation payload/Core/fixture semantics unchanged. | Hosted gates + complete native Windows ARM64 validation pass at one exact corrected checkout; new annotated validation tag and evidence/state closeout. Future provider traffic still requires separate explicit Director authorization. |
| Q-E0A-03 | Engineering / Evidence | BLOCKED | Complete the E0-A same-model isolated-cast reference evidence package. | A usable, separately authorized and machine-validated provider route; frozen fixture/configuration provenance. | E0-A closes with auditable run evidence and the reference configuration required by later controls. |
