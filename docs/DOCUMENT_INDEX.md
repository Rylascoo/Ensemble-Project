...
This index helps a fresh engineering chat find the small set of current entry surfaces. `tools/document-census.py` intentionally excludes this file from inbound-reference authority calculations, so a link here cannot make an otherwise historical document current.
...
3. `docs/ENGINEERING_HYGIENE_CONSTITUTION.md` — engineering and repository-surface law.
...
- `docs/BRANCH_ARCHIVE_2026_09.md` — historical branch-archive snapshot; current branch truth must be resolved from GitHub and `CURRENT_STATE.md`.

Historical blueprints/evidence remain available in repository history and archive surfaces. Their presence does not advance current authority.
