...
`docs/PROJECT_REASONING_TASK_SCOPE_OPTIMIZATION_PROTOCOL.md` — active v0.1 on authoritative `main`
...
The ship-plan promotion after Patch 0015 is documentation-only. No executable Core/test/fixture authority changed after the Patch 0015 machine checkpoint.
...
The prior transition branch also contained unrelated workflow/promotion documentation and incorrectly labelled a branch-local process commit as `main`. Proposal 0.4 is recreated on a clean branch from actual `main` and carries no such dependency.
...
### 0.4 — clean authority + testability correction

- rebased/recreated the candidate from actual promoted `main` `a2b1458509e595847360aaa0729e90a99eb98063`;
- removed dependency on unrelated branch-local workflow/promotion commits and correctly references authoritative Sol-High protocol v0.1;
- retained Proposal 0.3 causal semantics while simplifying repetitive prose;
...
- preserved the phase-two failure/adoption law through closed-construction, structural delegation, exception normalization and immutable-token tests.
