...
GitHub records E0-A Preparation Blueprint 0.2 as approved, but the full originating Prep 0.2 document is not currently present in the engineering repository. Do not reconstruct missing historical text and then treat the reconstruction as original authority.

After explicit approval, this document becomes the **canonical GitHub implementation specification for the Missing Raft fixture subset only**. It does not replace unrelated approved Prep 0.2 contracts for Context Composer, Performer, Director, Integrity Validator, Interpreter, State Authority, run protocol, evaluation, or other later H1/H2/H3 work. Before implementing those later surfaces, their relevant approved contract must likewise be checkpointed in GitHub rather than recovered from chat memory during coding.

This closes the current fixture-continuity gap without manufacturing a false historical artifact.
...
## 4. Mechanical law versus semantic-source review
...
## 5. Canonical source and identity
...
## 6. Scene and Character identity
Scene ID:
- `SCENE-MISSING-RAFT`
...
## 7. Exact Production-authoritative catalog
...
`HT-RAFT-DETERIORATED` establishes objective condition only. Marlowe's perception remains separately authoritative as his Character-owned Observation.
...
These are opening Scene conditions intended by the approved experiment to be common rather than private. **Patch 0002.2 does not implement or prove their disclosure.** Patch 0004 Access Control must explicitly preserve that intent.

`SCENE-NO-EXTERNAL-COUNTDOWN` preserves the absence of storm/rescue-timer pressure without duplicate countdown concepts.
...
Base authored truth/state records may have empty provenance when not derived from another fixture record.

## 8. Exact Character catalog
Ownership is structural because Character records are nested under their owner. Every collection below is frozen by exact ID set.

All three `circumstance` arrays are exactly empty in fixture 0.1.0. Immediate shared conditions belong in SceneState; approved Character-specific intent belongs in Goal/belief/suspicion. This avoids duplicating those facts merely to populate Circumstance. The Character model still preserves the Circumstance slot for fixtures that actually earn distinct immediate Character state.
...
Whether loss would otherwise have occurred is `UP-RH001-CACHE-WOULD-BE-LOST`, not HistoricalTruth.
...
## 10. Relationship memories and E0-D surface
...
No Wren RH-001 memory is invented because approved RH-001 meaning does not require one for her.
...
For later E0-D relationship omission, this set is an **exclusion surface for otherwise matched context construction**. It never deletes fixture records, changes Production truth, alters fixture version, or rewrites provenance. Thus E0-D changes context only while retaining the same frozen fixture.

## 11. Chronology
Chronology is semantically ordered but need not contain every HistoricalTruth record. It contains past events whose relative ordering matters to the experiment.
...
7. `HT-MARLOWE-RETURNED-SHORELINE`

This protects the approved dramatic evidence chain: relationship history precedes the current event; securing/deterioration precede release; release precedes loss; Wren's later return observation concerns Marlowe's return after the loss event, never the release itself.

`HT-MARLOWE-NOT-WARNED` and `HT-CURRENT-DID-NOT-RELEASE-MOORING` remain authoritative non-event/causal facts and do not need separate chronology entries.

## 12. Provenance is not access
...
- An allowed record may enter context without exposing inaccessible provenance source content or IDs unless those sources are independently permitted.
...
## 13. Contract implementation shape
...
- chronology matches the seven-ID sequence exactly;
...
## 16. Source-content review gate
Before machine validation, explicitly review `fixtures/missing-raft/missing-raft-0.1.0.json` against Section 9 and the approved fixture contract:
...
Only a source-reviewed fixture proceeds to ARM64 validation. Patch 0003 replaces this review-only immutability boundary with semantic canonical bytes + SHA-256.
...
## 18. Exit gate
Before merge:
1. this blueprint is explicitly approved as the canonical GitHub Missing Raft fixture-subset specification;
2. source-content review passes;
3. Harness/Core build passes with warnings-as-errors on target Windows ARM64;
4. entire Core test suite passes;
5. Harness validates `fixtures/missing-raft/missing-raft-0.1.0.json` as `ensemble.e0.missing-raft@0.1.0`;
6. generic smoke fixture remains valid through generic path;
7. direct Missing Raft validation rejects generic smoke fixture;
